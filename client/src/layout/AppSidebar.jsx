...
const navItems = [
    ...
    { icon: <UserCircleIcon />, name: "Patients", path: "/patients" },
    { icon: <ListIcon />, name: "Visits", path: "/visits" },
    { icon: <TableIcon />, name: "Billing", path: "/billing" },
    { icon: <PieChartIcon />, name: "Reports", path: "/reports" },
    { icon: <PageIcon />, name: "Templates", path: "/templates" },
    { icon: <MedicineIcon />, name: "Medicines", path: "/medicines" },
];
...
const AppSidebar = () => {
    ...
    const renderLink = (nav) => (
        <li key={nav.name}>
            <Link to={nav.path} className={`menu-item group ${isActive(nav.path) ? "menu-item-active" : "menu-item-inactive"} lg:justify-start`}>
        ...
        >
                    ...
                    <div className="flex flex-col gap-4">
                        <div>
                            <ul className="flex flex-col gap-4">
                                ...
                                {isAdmin && adminItems.map(renderLink)}
                            </ul>
                        </div>
                    </div>
                </nav>
            ...
            <div className="py-4 border-t border-gray-200 dark:border-gray-800">
                <div className="flex items-center justify-between px-4">
                    <Link to="/my-account" title="My Account" className="text-gray-600 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400 transition-colors">
